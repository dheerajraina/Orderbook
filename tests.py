...
def test_order_book_display():
    ...
    def push_buy_orders():
        for i in range(1000000):
            ...
            new_order = Order(
                order_id=f'buy{i}', price=price, quantity=quantity, side='buy')
    ...
    def push_sell_orders():
        for i in range(1000000):
            ...
            quantity = random.randint(0, 1000)
            new_order = Order(
                order_id=f'sell_{i}', price=price, quantity=quantity, side='sell')
            order_book.handle_new_order(new_order)
            order_book.display_order_book()
